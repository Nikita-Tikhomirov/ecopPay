...
function is_subcategory_of($parent_id) {
    // Проверяем, находится ли текущая страница на рубрике
    if (is_category()) {
        $category = get_queried_object();
        // Проверяем, есть ли у рубрики родительская рубрика
        if ($category->category_parent == $parent_id) {
            return true;
        }
        // Если у рубрики есть родительская рубрика, проверяем ее родителей рекурсивно
...
// Пример использования: добавляем действие в хук wp_head, чтобы вывести сообщение в заголовке страницы, если она является дочерней рубрикой рубрики с ID 9

    if (is_subcategory_of(24)) {
        include(TEMPLATEPATH . '/catTemplates/professionsCustomSort.php');
        
    }else{
        include(TEMPLATEPATH . '/catTemplates/allCats.php');
    }
